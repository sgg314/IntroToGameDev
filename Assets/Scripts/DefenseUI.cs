...
public class DefenseUI : MonoBehaviour 
{
	...
	{
		healthText.text = "Core Health: " + towerScript.towerHealth + " of 5"; 			//Shows tower's health
        ...
        {
            youLose.text = "Chip Status: VIRUS ALERT!"; 	//Says the game over text
            youLose.color = Color.red;                  //Text changes to red
        } else {
            youLose.text = "Chip Status: OPTIMAL"; 	//Says the game over text
            youLose.color = Color.yellow;           //Text changes to red
        }
        if (towerScript.towerHealth <= 1) //If the variable is under or equal to 1...
        {
            healthText.color = Color.red; //Text changes to red
        } else {
            healthText.color = Color.green; //Text remains black otherwise
        }
    }
}