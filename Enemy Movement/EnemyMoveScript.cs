...
public class EnemyMoveScript : MonoBehaviour {
	...
	void Start () {
		...
		enemyStartX = 15;//enemyScript.enemySpawnX;			//Makes enemies start at the coordinates of the tower they are spawned at
		enemyStartY = 0;//enemyScript.enemySpawnY;			//Makes enemies start at the coordinates of the tower they are spawned at
	...
	void OnTriggerEnter2D(Collider2D thingThatHitMe) 		//Function triggers whenever something touches its collider. It even figures out what hit it.
	{
		...
		if (thingThatHitMe.gameObject.name == "Tower") { 		//The following code only occurs if the object touches the player
		...
		if (thingThatHitMe.gameObject.name == "Player") { 		//The following code only occurs if the object touches the tower
